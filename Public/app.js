 ...
 * 
 * Key features:
 * - Web Worker architecture to prevent UI freezing
 * - Multi-threaded prediction calculations
 * - Memory-optimized algorithms
 * - Progressive enhancement for all browsers
 * - Advanced multiplier application logic
...
function initAdvancedMode() {
  ...
  const predictionEngine = new PredictionEngine({
    ...
    onWorkerReady: () => {
      console.log('тЬЕ Prediction engine initialized with Web Worker');
...
function initBasicMode() {
  console.log('тЪая╕П Initializing in basic mode (older browser)');
...
class PredictionEngine {
  ...
  initializeWorker() {
    // Create Web Worker from inline code (avoids separate file issues on Netlify)
    const workerCode = `
      ...
      function calculatePrediction(multipliers, accuracy, multiplierRange) {
        ...
        // Calculate prediction (simplified for demo)
  ...
  fallbackToBasicEngine() {
    console.log('тЪая╕П Falling back to basic prediction engine');
    this.worker = null;
    this.isWorkerReady = false;
...
class PerformanceMonitor {
  ...
  logPerformanceStats() {
    ...
    console.log(`ЁЯУК Performance: ${avgFps.toFixed(1)} FPS | Jank: ${this.jankCount} events`);
...
// Initialize UI manager globally for event handlers
document.addEventListener('DOMContentLoaded', () => {
  if (typeof PredictionEngine !== 'undefined' && window.predictionEngine) {
    window.uiManager = new UIManager(window.predictionEngine);
