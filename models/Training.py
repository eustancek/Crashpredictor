...
SUPABASE_KEY = os.getenv("SUPABASE_KEY", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZhd2N1d2NxZnd6dmRvYWxjb2N4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA4NDY3MjYsImV4cCI6MjA2NjQyMjcyNn0.5NCGUTGpPm7w2Jv0GURMKmGh-EQ7WztNLs9MD5_nSjc")  # Your actual Anon key here

# Fixed PostgreSQL connection string (Transaction Pooler for IPv4)
engine = create_engine(
    f"postgresql://{SUPABASE_KEY.split('.')[0]}:"  # Username is first part of the key
    f"{SUPABASE_KEY}@"  # Full key as password
    f"aws-0-ca-central-1.pooler.supabase.com:6543/postgres?sslmode=require"  # Correct host/port
)
...
class CrashTrainer:
    ...
    def train(self, data=None, epochs=10, batch_size=32, incremental=False):
        try:
            ...
            kf = KFold(n_splits=3)  # Reduced to handle small datasets
            histories = []
