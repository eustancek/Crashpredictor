...
SUPABASE_KEY = os.getenv("SUPABASE_KEY", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZhd2N1d2NxZnd6dmRvYWxjb2N4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA4NDY3MjYsImV4cCI6MjA2NjQyMjcyNn0.5NCGUTGpPm7w2Jv0GURMKmGh-EQ7WztNLs9MD5_nSjc")  # Replace with your actual key

# Fixed PostgreSQL connection string (use full key as password, no 'db.' subdomain)
engine = create_engine(
    f"postgresql://postgres:{SUPABASE_KEY}@"
    f"{SUPABASE_URL.replace('https://', '').split('.')[0]}.supabase.co:5432/postgres?sslmode=require"
)
...
class DataProcessor:
    def load_and_clean_data(self):
        try:
            # Query Supabase (fallback to mock data if empty)
            query = """
            ...
            # Remove outliers but ensure mock data isn't affected
    ...
    def _normalize(self, data):
        return (data - data.min()) / (data.max() - data.min() + 1e-7)  # Add epsilon to avoid division by zero
...
class CrashPredictor:
    ...
    def _load_or_build_model(self):
        ...
        # Fixed model architecture with Input layer
