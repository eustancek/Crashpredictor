...
SUPABASE_KEY = os.getenv("SUPABASE_KEY", "YOUR_SUPABASE_ANON_KEY")  # Replace with your actual key

# Fixed PostgreSQL connection string (removed 'db.' subdomain)
engine = create_engine(
    f"postgresql://{SUPABASE_KEY.split('.')[0]}@{SUPABASE_URL.replace('https://', '')}:5432/postgres?sslmode=require"
)
...
class CrashTrainer:
    ...
    def train(self, data=None, epochs=10, batch_size=32, incremental=False):
        try:
            ...
            kf = KFold(n_splits=3)  # Reduced splits to avoid sample不足
            histories = []
...
class DataProcessor:
    def load_and_clean_data(self):
        try:
            ...
            LIMIT 200  -- Ensure sufficient data for training
            """
...
class DataAugmenter:
    def augment(self, data):
        X_aug = []
        y_aug = []
        # Basic augmentation (add more if needed)
        X_aug.extend(data["X"].tolist())
        y_aug.extend(data["y"].tolist())
        X_aug.extend((data["X"] * (1 + np.random.normal(0, 0.01, data["X"].shape))).tolist())
        return {"X": np.array(X_aug), "y": np.array(y_aug)}

# New helper function to ensure models directory exists
